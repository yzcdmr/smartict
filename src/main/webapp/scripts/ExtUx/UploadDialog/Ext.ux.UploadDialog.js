...
Ext.ux.UploadDialog.BrowseButton = Ext.extend(Ext.Button, {
    ...
    createInputFile: function () {
        //var button_container = this.el.child(".x-btn-center"); //2.x锟芥本ext使锟斤拷(<td class="x-btn-center">) swb update 20091223
        var button_container = this.el.child(".x-btn-mc"); //3.x锟芥本ext使锟斤拷(<td class="x-btn-mc">) swb update 20091223
            ...
            listeners: {
                rowdblclick: function (t) {
                    ...
                    if (!Ext.isEmpty(record.get('id'))) {
                        ...
                        document.location = 'download.action?fileId=' + record.get('id');
                    }
                }
            }
        });
...
//            iconCls: 'close'
    ...
    isPermittedFile: function (browse_btn) {
        ...
        if (this.isPermittedFileType(filename)) {
            ...
            Ext.Msg.alert(
            this.i18n.error_msgbox_title, String.format(
            this.i18n.err_file_type_not_permitted, filename, this.permitted_extensions.join(this.i18n.permitted_extensions_join_str)));
            result = false
        }
        return result
    },
    ...
    updateProgressBar: function () {
        if (this.is_uploading) {
            ...
            this.i18n.progress_uploading_text, this.initial_queued_count - queued, this.initial_queued_count))
        } else {
...
//                Ext.fly(tb.x_buttons.indicator.getEl()).replaceClass("stop", "processing");
                tb.x_buttons.upload.setIconClass("ext-ux-uploaddialog-uploadstopbtn");
//                tb.x_buttons.upload.setIconClass("start");
...
//                Ext.fly(tb.x_buttons.indicator.getEl()).replaceClass("processing", "stop");
...
//                tb.x_buttons.upload.setIconClass("start");
    ...
    prepareNextUploadTask: function () {
        ...
        store.each(function (r) {
            if (!record && r.get("state") == Ext.ux.UploadDialog.FileRecord.STATE_QUEUE) {
                ...
                if (r && r.get("input_element") && !Ext.isEmpty(r.get("input_element"))) {
    ...
    removeFiles: function (file_records) {
        var p = this;
        var delIds = "";
        Ext.MessageBox.confirm('SİL', 'Dosyayı çıkarmak istediğinizden emin misiniz?', function (btn) {
            if (btn == 'yes') {
...
//                    Ext.Ajax.request({
//                        url: '../rest/sablonlar/saveOrUpdateSablonlar.ajax',//'fileupload!deleteAffixsByAjax.action',
//                        method: 'POST',
...
//                }
    ...
    updateRecordState: function (data) {
        if ("success" in data.response && data.response.success) {
            ...
            //data.record.set('id', data.response.collection[0]);
            // }
        } else {
    ...
    onAjaxSuccess: function (response, options) {
        ...
        try {
            ...
            if (this.hiddenFieldId) {
                ...
                if (Ext.isEmpty(v)) Ext.getCmp(this.hiddenFieldId).setValue(json_response.collection[0]);
                else Ext.getCmp(this.hiddenFieldId).setValue(v + "," + json_response.collection[0]);
            }
        } catch (e) {}
        var data = {
    ...
    isNotEmptyQueue: function () {
        return this.grid_panel.getStore() && this.grid_panel.getStore().getCount() > 0
    },
...
p.i18n = {
    ...
    note_col_width: 150,
    add_btn_text: "Ekle",
    add_btn_tip: "Yüklemek istediğiniz dosyayı ekleyiniz",
    remove_btn_text: "Sil",
    remove_btn_tip: "Seçili dosyayı listeden siliniz.",
    reset_btn_text: "Sıfırla",
    reset_btn_tip: "Listeyi sıfırla.",
    upload_btn_start_text: "Gönder",
    upload_btn_stop_text: "Dur",
    upload_btn_start_tip: "Listedeki dosyaları gönder.",
    upload_btn_stop_tip: "Yüklemeyi durdur.",
    close_btn_text: "Kapat",
    close_btn_tip: "Paneli kapat.",
    progress_waiting_text: "Lütfen bekleyiniz...",
    progress_uploading_text: "Yükleniyor...",
    error_msgbox_title: "Hata",
    permitted_extensions_join_str: ",",
    err_file_type_not_permitted: "Yüklemek istediğiniz dosya <b>uygun formatta değil.</b><br/>Lütfen istenen uzantıda bir dosya gönderiniz...",
    note_queued_to_upload: "Yükleme için sıraya kondu.",
    note_processing: "Gönderiliyor...",
    note_upload_failed: "Dosya gönderme başarısız...",
    note_upload_success: "Tamam.",
    note_upload_error: "Gönderme hatası.",
    note_aborted: "Kullanıcı tarafından iptal edildi.",
    note_canceled: "Yükleme iptal edildi."
};