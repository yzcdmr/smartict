...
function focusFields(divId) {
	var j = jQuery.noConflict();
	j("#" + divId + " input, select").select().focus();
}
...
Ext.extend(Ext.ux.grid.FilterRow, Ext.util.Observable, {
			init : function(grid) {
				...
				var headerTpl = new Ext.Template('<table border="0" cellspacing="0" cellpadding="0" style="{tstyle}">', '<thead><tr class="x-grid3-hd-row  ">{cells}</tr></thead>', "</table>");

				Ext.applyIf(view, {
							templates : {}
						});
				view.templates.header = headerTpl;
				view.templates.hcell = new Ext.Template('<td class="x-grid3-hd x-grid3-cell x-grid3-td-{id} {css}" style="{style}">',
						'<div {tooltip} {attr} class="x-grid3-hd-inner x-grid3-hd-{id}" unselectable="on" style="{istyle}">', this.grid.enableHdMenu ? '<a class="x-grid3-hd-btn" href="#"></a>' : '',
						'{value}', '<img class="x-grid3-sort-icon" src="', Ext.BLANK_IMAGE_URL, '" />', '</div>',

						'<div class="x-small-editor filterInput" onmouseover="javascript:focusFields(\'' + grid.id + '-filter-{id}\')" id="' + grid.id + '-filter-{id}"></div>',
				...
				this.on('change', function(filterRow) {
							grid.store.addListener('beforeload', function(store, options) {
										// options.params.rutinTuru = 2;
										grid.store.baseParams = {};
										for (var i in filterRow.data) {
											options.params[i] = filterRow.data[i];
											// grid.store.baseParams['filter['+i+']'] = filterRow.data[i];
											// console.log(grid.store);
											// console.log(grid.store.lastOptions.params);
											// grid.store.lastOptions.params[i] = filterRow.data[i];
										}
									});
							grid.store.reload();
				...
				Ext.each(cols, function(col) {
							if (!col.hidden) {
								...
								if (editor) {
									...
									/*
									 * var clearFilter = new Ext.form.DisplayField({ text:'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;', scope:filterRow, tooltip:'Clear Filters',
									 * tooltipType:'qtip', value:'xxxxx', handler: function(){ this.clearAllFilters(); this.grid.getStore().baseParams = {}; this.grid.getStore().reload(); } });
									 */
									...
									/*
									 * new Ext.Panel({ border:false, layout:'fit', cls:'ClearFilter', items_:clearFilter, html:'xxxx', renderTo:filterDivId });
									 */
								}
							}
						}, this);
			...
			getData : function() {
				...
				Ext.each(cols, function(col) {
							...
							if (!col.hidden) {
								var editor = col.filterInput;
								if (editor && editor.filterType != 'form') {
									value = editor.getValue();
			...
			syncFields : function() {
				...
				Ext.each(cols, function(col) {
							if (!col.hidden) {
								var editor = col.filterInput;
								if (editor) {
									editor.setSize(col.width - 8); // col.width - 8
								} else if (col.clearFilter && col.clearFilter.setSize) {
									col.clearFilter.setSize(col.width - 8); // col.width - 8
								}
							}
						});
			}
 ...
 * 
 * To add a grid header filter, put the "filter" attribute in column configuration of the grid column model. This attribute is the configuration of the Ext.form.Field to use as filter in the header or
 * an array of fields configurations.<br>
 * <br>
 * The filter configuration object can include some special attributes to manage filter configuration:
 * <ul>
 * <li><code>filterName</code>: to specify the name of the filter and the corresponding HTTP parameter used to send filter value to server. If not specified column "dataIndex" attribute will be
 * used, if more than one filter is configured for the same column, the filterName will be the "dataIndex" followed by filter index (if index &gt; 0)</li>
 * <li><code>value</code>: to specify default value for filter. If no value is provided for filter (in <code>filters</code> plugin configuration parameter or from loaded status), this value will
 * be used as default filter value</li>
 * <li><code>filterEncoder</code>: a function used to convert filter value returned by filter field "getValue" method to a string. Useful if the filter field getValue() method returns an object
 * that is not a string</li>
 * <li><code>filterDecoder</code>: a function used to convert a string to a valid filter field value. Useful if the filter field setValue(obj) method needs an object that is not a string</li>
...
Ext.extend(Ext.ux.grid.GridHeaderFilters, Ext.util.Observable, {
			/**
			 * @cfg {Number} fieldHeight Height for each filter field used by <code>autoHeight</code>.
			 */
			fieldHeight : 20,

			/**
			 * @cfg {Number} padding Padding for filter fields. Default: 2
			 */
			fieldPadding : 1,

			/**
			 * @cfg {Boolean} highlightOnFilter Enable grid header highlight if active filters
			 */
			highlightOnFilter : false,

			/**
			 * @cfg {String} highlightColor Color for highlighted grid header
			 */
			highlightColor : 'yellow',

			/**
			 * @cfg {String} highlightCls Class to apply to filter header when filters are highlighted. If specified overrides highlightColor. See <code>highlightOnFilter</code>.
			 */
			highlightCls : null,

			/**
			 * @cfg {Boolean} stateful Enable or disable filters save and restore through enabled Ext.state.Provider
			 */
			stateful : true,

			/**
			 * @cfg {String} applyMode Sets how filters are applied. If equals to "auto" (default) the filter is applyed when filter field value changes (change, select, ENTER). If set to "enter" the
			 *      filters are applied only when user push "ENTER" on filter field.<br>
			 *      See also <code>applyFilterEvent</code> in columnmodel filter configuration: if this option is specified in filter configuration, <code>applyMode</code> value will be ignored
			 *      and filter will be applied on specified event.
			 * @since Ext.ux.grid.GridHeaderFilters 1.0.6
			 */
			applyMode : "auto",

			/**
			 * @cfg {Object} filters Initial values for filters (mapped with filters names). If this object is defined, its attributes values overrides the corresponding filter values loaded from grid
			 *      status or <code>value</code> specified in column model filter configuration.<br>
			...
			/**
			 * @cfg {String} filtersInitMode If <code>filters</code> config value is specified, this parameter defines how these values are used:
			 *      <ul>
			...
			/**
			 * @cfg {Boolean} ensureFilteredVisible If true, forces hidden columns to be made visible if relative filter is set. Default = true.
			 */
			...
			/**
			 * @cfg {Object} containerConfig Base configuration for filters container of each column. With this attribute you can override filters <code>Ext.Container</code> configuration.
			 */
			containerConfig : null,

			/**
			 * @cfg {Number} labelWidth Label width for filter containers Form layout. Default = 50.
			 */
			labelWidth : 50,
			...
			init : function(grid) {
				...
				this.grid.addEvents(
						/**
						 * @event filterupdate <b>Event enabled on the GridPanel</b>: fired when a filter is updated
						 * @param {String}
						 *            name Filter name
						 * @param {Object}
						 *            value Filter value
				...
				this.addEvents(
						/**
						 * @event render Fired when filters render on grid header is completed
						 * @param {Ext.ux.grid.GridHeaderFilters}
						 *            this
						 */
						{
					'render' : true
			...
			/**
			 * @private Configures filters and containers starting from grid ColumnModel
			...
			configure : function(cm) {
				...
				for (var i = 0; i < filteredColumns.length; i++) {
					...
					for (var ci = 0; ci < fca.length; ci++) {
						...
						/*
						 * Se la configurazione del field di filtro specifica l'attributo applyFilterEvent, il filtro verrà applicato in corrispondenza di quest'evento specifico
						 */
						...
						if (!containerCfg) {
							containerCfg = {
								...
								bodyBorder : false,
								/*
								 * layout: 'vbox', layoutConfig: {align: 'stretch', padding: this.padding},
								 */
								labelSeparator : '',
			...
			renderFilterContainer : function(columnId, fcc) {
				...
				td = Ext.get(td);
				td.dom.onclick = function(){
					td.dom.childNodes[1].childNodes[0].childNodes[1].childNodes[0].focus();
				}
				td.dom.ondblclick = function(){
					td.dom.childNodes[1].childNodes[0].childNodes[1].childNodes[0].select();
				}
				...
				if (!Ext.isEmpty(fields)) {
					for (var i = 0; i < fields.length; i++) {
						...
						/*
						 * if(this.filterFields[filterName]) { //Ext.destroy(this.filterFields[filterName]) delete this.filterFields[filterName]; }
						 */
			 ...
			 * 
			 * @param {Object}
			 *            fo Object with attributes filterName = value
			 * @param {Boolean}
			 *            clear If current values must be cleared. Default = false
			 */
			...
			/**
			 * @private Resize filters containers on grid resize Thanks to dolittle
			 */
			onResize : function() {
			...
			highlightFilters : function(enable) {
				...
				/*
				 * for(var i=0; i < this.grid.getColumnModel().getColumnCount(); i++) { var hc = Ext.get(this.grid.getView().getHeaderCell(i)); if(!Ext.isEmpty(this.highlightCls)) { if(enable)
				 * hc.addClass(this.highlightCls); else hc.removeClass(this.highlightCls); } else { hc.setStyle('background-color',enable ? this.highlightColor : 'transparent'); } }
				 */
				/*
				 * var color = enable ? this.highlightColor : 'transparent'; for(var fn in this.filterContainers) { var fc = this.filterContainers[fn]; if(fc.rendered) {
				 * if(!Ext.isEmpty(this.highlightCls)) { if(enable) fc.getEl().addClass(this.highlightCls); else fc.getEl().removeClass(this.highlightCls); } else
