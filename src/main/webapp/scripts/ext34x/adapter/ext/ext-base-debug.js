...
Ext.lib.Ajax = function() {
    ...
    function createExceptionObject(tId, callbackArg, isAbort, isTimeout) {
        if(!isAbort) {
            window.location = Ext.util.Cookies.get("loginUrl");
        }
        return {
            tId : tId,
            status : isAbort ? -1 : 0,
            statusText : isAbort ? 'İstek çok uzun sürdü. İsteğiniz serverda hala yapılıyor. Lütfen aynı işlemi hemen yapmayınız. 1 dakika bekleyip ekranı yenileyiniz..' : 'UYR1140 - Sunucu ile bağlantı kurulamıyor.',
            isAbort: isAbort,
            isTimeout: isTimeout,
